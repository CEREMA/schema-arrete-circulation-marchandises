...
Schéma des objets importants

Spécification du fichier d'échange relatif aux objets importants gérés par les collectivités.

- Schéma créé le : 06/29/18
- Site web : https://github.com/mon-orga/schema-objets-importants
- Version : 0.1.1
- Clé primaire : `id`
...
|-|-|-|-|-|
|id|nombre entier|Un identifiant unique de l'objet.|42|Valeur obligatoire, Valeur minimale : 1|
|date_creation|date|La date de création de l'objet, au format ISO 8601 AAAA-MM-DD.|2020-02-27|Valeur obligatoire|
|nom|chaîne de caractères|Le nom de l'objet.|Brouette|Valeur obligatoire|
|classification|liste|Classification de l'objet.|Classification A||
...
|GEOM_JSON (Géométrie)|GéoJSON|Géométrie de la ligne au format GeoJSON (de l'anglais Geographic JSON, signifiant littéralement JSON géographique, est un format ouvert d'encodage d'ensemble de données géospatiales simples utilisant la norme JSON (JavaScript Object Notation).|||
...
|VEH_CQA (Vignettes crit'air)|chaîne de caractères|Vignettes crit'air|['1', '2', '3']||
...
|ARR_URL (Adresse internet par laquelle accéder à l'arrêté, et donc au règlement)|chaîne de caractères|Identifiant du [Système d'Identification du Répertoire des Etablissements](https://fr.wikipedia.org/wiki/Syst%C3%A8me_d%27identification_du_r%C3%A9pertoire_des_%C3%A9tablissements) (SIRET) de la collectivité sur le territoire de laquelle sont situés les équipements collectifs publics répertoriés dans le jeu de données. Il est composé de 9 chiffres SIREN + 5 chiffres NIC d’un seul tenant.|https://carte.st-paul-les-dax.fr/wp-content/uploads/2020/06/AM-10248.pdf|Valeur optionnelle, Motif : `^(https|http)?://(?:[a-z0-9\-]+\.)+[a-z]{2,6}(?:/[^/#?]+)+`|
...
|ARR_DATE_CREATION (Date de création de l'arrêté)|date|Date de création ou de mise à jour de l'arrêté, au format ISO 8601 AAAA-MM-DD.|2021-04-30|Valeur obligatoire|
|ARR_MAJ (Arrêté mis à jour ?)|??Boolean??|Valeurs considérées comme vraies : ['true', 'True', 'TRUE', '1', 'vrai', 'Vrai', 'VRAI']Valeurs considérées comme fausses : ['false', 'False', 'FALSE', '0', 'faux', 'Faux', 'FAUX']Spécifie si l'arrêté a été l'objet d'une mise à jour. Dans ce cas, remplir la nouvelle référence de l'arrêté dans `ARR_REF`.|TRUE|Valeur obligatoire|
